...
/**
 * Modal for scheduling a future phone call as a commercial action.
 *
 * Creates a {@link CreateCommercialActionRequest} of type CALL.
 * Includes priority, due date, optional reminder and description.
 * The accent color is blue (#2563eb), consistent with the call theme.
 */
@Component({
  selector: 'app-schedule-call-modal',
  imports: [FormsModule, ModalLayoutComponent],
  templateUrl: './schedule-call-modal.component.html',
  styleUrl: './schedule-call-modal.component.scss'
})
export class ScheduleCallModalComponent implements OnInit, OnDestroy {
  ...
  submit(): void {
    ...
    const body: CreateCommercialActionRequest = {
      type:               CommercialActionType.CALL,
    ...
    this.api.create(body).subscribe({
      next:  () => { this.saving.set(false); this.feedback.showSuccess('Appel planifié.'); this.created.emit(); },
