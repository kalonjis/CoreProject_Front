...
export class TwoFactorOverviewComponent implements OnInit {
  ...
  private loadTwoFactorMethods(): void {
    ...
    this.twoFactorApi.getSettings()
      ...
      .subscribe((methods: TwoFactorMethod[]) => this.twoFactorMethods.set(methods.filter(m => m.type !== "BACKUP_CODES")));
  }
  ...
  getEnabledMethodsCount(): number {
    return this.twoFactorMethods().filter(m => m.isEnabled && m.type != "BACKUP_CODES").length;
  }
