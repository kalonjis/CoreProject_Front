...
export class OAuthButtonComponent {
  ...
  private readonly configs: Record<OAuthProvider, OAuthConfig> = {
    github: {
      svg: `<svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
      ...
      url: 'https://localhost:8443/oauth2/authorization/github'
    },
    ...
    google: {
      ...
      url: 'https://localhost:8443/oauth2/authorization/google'
    },
    ...
    microsoft: {
      ...
      url: 'https://localhost:8443/oauth2/authorization/microsoft'
    }
  };

  /**
   * Gets the OAuth URL for the current provider
   */
  get oauthUrl(): string {
    return this.configs[this.provider].url;
  }

  /**
   * Gets the SVG icon for the current provider (sanitized for security)
   */
  get svgIcon(): SafeHtml {
    return this.sanitizer.bypassSecurityTrustHtml(this.configs[this.provider].svg);
  }

  /**
   * Gets the label text for the current provider
   */
  get label(): string {
    return this.configs[this.provider].label;
  }
}
